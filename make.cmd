...
SET UPX_PATH=\programs\development\rce\tools\packers
...
REM --------------------------------------------------------------------
SET ML_ARGS=/c /coff /nologo /Fo"%PROJECT_BIN%\%FILENAME%.obj" "%PROJECT%\%FILENAME%.asm"
...
IF %BUILD_DLL%==TRUE (
    ...
    IF %BUILD_LIB%==TRUE (
        ...
        SET EXT=exe
        SET LINK_ARGS=/nologo /SUBSYSTEM:WINDOWS /OUT:"%PROJECT_BIN%\%FILENAME%.exe" "%PROJECT_BIN%\%FILENAME%.obj" %RSRC_PARAM%
    )
)
